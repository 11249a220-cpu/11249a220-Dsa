...
To implement a Doubly Linked List in C that supports basic operations such as:

Creating a doubly linked list

Inserting a node (at beginning, end, or any position)
...
If list is not empty, set head->prev = newnode.
...
If the list is empty → print “List empty”.

Traverse the list to find the node to delete (temp).

If temp->prev != NULL, set temp->prev->next = temp->next.

Else update head = temp->next.
...
Start from the last node by traversing forward from head.
